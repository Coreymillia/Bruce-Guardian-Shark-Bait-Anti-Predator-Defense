...
void analyzeThreats() {
    ...
    for(auto& device : trackedDevices) {
        ...
        if(device.riskScore >= ATTACK_DETECTION_THRESHOLD && !device.isMarkedMalicious) {
            ...
            Serial.println("🚨 SHARK DETECTED: " + getAttackTypeName(device.suspectedAttack) + 
...
void AntiPredatorMenu::optionsMenu() {
    options = {
        {"Threat Monitor", [=]() {
            ...
            padprintln("🦈 PROTECTING AGAINST SHARKS 🦈");
            padprintln("");
            padprintln("Active defenses:");
            padprintln("✅ Beacon spam detector (>3/s)");
            padprintln("✅ Evil twin hunter (multi-SSID)");
            padprintln("✅ Karma attack sentinel");
            padprintln("✅ Deauth storm monitor (>2/s)");
            padprintln("✅ Probe flood detector (>8/s)");
            padprintln("");
        ...
        {"Shady WiFis", [=]() {
            drawMainBorderWithTitle("🕵️ SHADY WIFI SCANNER 🕵️");
        ...
        {"Counter Attack", [=]() {
            ...
            std::vector<Option> counterOptions = {
                {"Karma Attack Poisoning", [=]() {
                    drawMainBorderWithTitle("🎯 KARMA ATTACK POISONING 🎯");
                ...
                {"Evil Twin Disruption", [=]() {
                    drawMainBorderWithTitle("🎭 EVIL TWIN DISRUPTION 🎭");
                ...
                {"Evil Counter Attack", [=]() {
                    drawMainBorderWithTitle("🌐 EVIL COUNTER ATTACK 🌐");
                        ...
                        for(int i = 0; i < networks; i++) {
                            ...
                            if(WiFi.encryptionType(i) == WIFI_AUTH_OPEN && ssid.length() > 0) {
                                ...
                                if(WiFi.status() == WL_CONNECTED) {
                                    ...
                                    tft.println("🌐 Loading portal page...");
                                    ...
                                    if(client.connect("192.168.4.1", 80)) {
                                        ...
                                        if(response.indexOf("Google Account") >= 0 || response.indexOf("login-form") >= 0) {
                                            tft.setTextColor(TFT_CYAN);
                                            tft.println("📄 Portal page loaded!");
                                            
                                            // Step 2: Submit form like a real browser
                                            delay(1000);
                                            tft.setTextColor(TFT_ORANGE);
                                            tft.println("📝 Filling form...");
                                                ...
                                                if(client.connect("192.168.4.1", 80)) {
                                                    ...
                                                    if(credsResponse.indexOf("Ya damn Fool") >= 0 || 
                                                       ...
                                                       credsResponse.indexOf("email") >= 0) {
                                                        ...
                                                        tft.setTextColor(TFT_YELLOW);
                                                        tft.println("⚠️ Checking credentials...");
                                                        delay(2000);
                                                    }
                                                }
                                            }
                ...
                {"Captive Portal Hijacking", [=]() {
                    drawMainBorderWithTitle("🚨 CAPTIVE PORTAL HIJACKING 🚨");
                ...
                {"Protective Victim Deauth", [=]() {
                    drawMainBorderWithTitle("🛡️ PROTECTIVE DEAUTH 🛡️");
        ...
        {"Card Skimmer Hunt", [=]() {
            drawMainBorderWithTitle("💳 CARD SKIMMER DETECTOR 💳");
            padprintln("Scanning for card skimmer networks");
            padprintln("and payment fraud devices...");
            padprintln("");
            padprintln("Detection signatures:");
            padprintln("🔍 ATM-related SSIDs");
            padprintln("🔍 Payment processor names");
            padprintln("🔍 Banking WiFi impersonation");
            padprintln("🔍 POS system backdoors");
            padprintln("🔍 Credit card capture APs");
            padprintln("");
            ...
            if(skimmersFound == 0) {
                padprintln("✅ No card skimmer networks");
                padprintln("detected in this area.");
                padprintln("");
                padprintln("Payment systems appear safe.");
            } else {
                padprintln("⚠️ " + String(skimmersFound) + " potential skimmers found!");
                padprintln("");
